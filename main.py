...
class UI(QtWidgets.QMainWindow):
    ...
    def startFunc(self):
        if Detection(self.path.currentText()) in cam1List():
            ID = []
            ID = idGenerate(Detection(self.path.currentText())[
                            0], (Detection(self.path.currentText()))[1], CAM_1_ID)
            self.log.addItem(
                f'Current Time : {current_time}    Camera Name: {CAM_1_ID}  Infor : {str(ID[1]), str(ID[2]),str(ID[3])}')
            self.IMGShow()
        elif Detection(self.path.currentText()) in cam2List():
            ID = []
            ID = idGenerate(Detection(self.path.currentText())[
                            0], (Detection(self.path.currentText()))[1], CAM_2_ID)
            self.log.addItem(
                f'Current Time : {current_time}    Camera Name: {CAM_2_ID} Infor : {str(ID[1]), str(ID[2]),str(ID[3])} ')
            self.IMGShow()
        else:
            ID = []
        ...
        query = "INSERT INTO Car (cameraId, twoDigits , color, otherDigits) VALUES (%s, %s, %s, %s)"
